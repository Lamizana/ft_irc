...
> Reproduction du sujet du projet ft_irc (école 42). Ce document sert uniquement de référence de l'énoncé ; le README principal présente le projet, son installation et son utilisation.
...
- Si cela se produit, votre projet sera considéré comme non fonctionnel (note de 0).
- Vous devez fournir un Makefile qui compilera vos fichiers sources. Il ne doit pas relink.
...
- IRC est un protocole utilisant TCP et optionnellement TLS.

1. **Aspects techniques:**
   - Un serveur IRC peut se connecter à d'autres serveurs IRC.
   - Un réseau IRC est un ensemble de serveurs connectés l'un à l'autre.
   - L'utilisateur utilise un logiciel IRC afin de se connecter à un des serveurs du réseau.
   - Le protocole ouvert est décrit par un RFC:
     - ***Requests for comments*** ("demandes de commentaires").
     - Documents décrivant les aspects et spécifications techniques d'Internet.

> RFC 1459 et RFC 2810 à RFC 2813

   - Un serveur IRC est géré par un ou plusieurs IrcOps (***IRC OPERATOR***):
     - Ils sont nommés par les administrateurs.

2. **Les canaux:**
   - ***Channel*** ("canal"):
     - Élément de base pour communiquer sur un réseau.
     - Un channel est défini par une liste d'utilisateurs connectés à celui-ci.
   - Pour rentrer dans un channel ou le créer:
```
/join <canal>
```
> Les canaux pouvant être vus de tous sur le réseau sont préfixés par `#`.
>
> Les canaux peuvent être locaux (1 seul serveur) et préfixés par `&`.

3. **Les modes:**
   - Options positionnées sur des canaux ou des utilisateurs.
   - Affectent leurs modes de fonctionnement, leurs privilèges et leurs interactions.

4. **Principales commandes:**
   - Les commandes IRC sont toujours définies par le caractère '/' placé en début de ligne.
   - Elle est envoyée comme message au canal actif.
   - Par exemple pour joindre un canal:
```
/join #canal
```
> Rejoint un canal public nommé canal.

5. **Bot IRC:**
   - Programme autonome connecté au réseau IRC qui réagit à des commandes ou événements.

6. **Les réseaux:**
   - Ensemble de serveurs IRC interconnectés (maillage). La communication entre serveurs n'est pas exigée pour ce projet.
</details>
...
- ***Direct Client-to-Client***, protocole utilisé par de nombreux clients IRC qui permet une connexion directe entre utilisateurs:
  - Permet d'envoyer des fichiers.
  - Permet de chatter plus rapidement et de manière plus sécurisée avec un autre utilisateur.
</details>
...
```
	...
	- type: spécifie le type de socket, SOCK_STREAM pour TCP.
	- protocol: définit le protocole, IPPROTO_TCP pour un socket TCP.
...
<details>
<summary>listen(): permet au socket d'écouter les connexions entrantes</summary>

```cpp
int listen(int sckt, int backlog);
```
	- sckt est le socket auquel les clients peuvent se connecter.
	- backlog est le nombre de connexions pouvant être gérées.
</details>
...
```
	- socket: socket duquel réceptionner les données.
	- buffer: tampon où stocker les données.
	- len: nombre maximal d'octets à réceptionner.
...
> [!CAUTION]
> Utiliser des descripteurs de fichiers non bloquants, il est possible d'utiliser des fonctions de lecture/récupération ou d'écriture/envoi sans poll() (ou équivalent), et le serveur ne doit pas être bloquant.
>
> Mais il consommera plus de ressources système.
>
> Lire/récupérer ou écrire/envoyer dans n'importe quel descripteur de fichier sans utiliser poll() (ou équivalent) ne valide pas le projet !
...
- Le client de référence doit pouvoir se connecter au serveur sans rencontrer d'erreur.
...
Cependant, implémenter les fonctionnalités suivantes :
	- être en mesure:
		- de s'authentifier
		- de définir un pseudonyme (changeable à tout moment)
...
```
- Permet au socket d'écouter les connexions entrantes
	- sckt est le socket auquel les clients peuvent se connecter.
	- backlog est le nombre de connexions pouvant être gérées.

# PARTIE BONUS

- Voici les fonctions supplémentaires que vous pouvez ajouter à votre serveur IRC pour qu'il ressemble encore plus à un véritable serveur IRC :
